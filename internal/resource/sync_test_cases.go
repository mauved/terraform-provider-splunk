...
// objectCheckFunc is a function that performs a check against a SyncGetter.
type objectCheckFunc func(t *testing.T, name string, s SyncGetter)
...
type syncObjectTestCase struct {
	...
	// sync is the SyncGetter object that is being tested. It must be set. It must be a SyncGetter (not just a Sync)
	// as checkFunc must be able to call sync.GetObject().
	sync SyncGetter
...
// the value retured by SyncGetter's GetObject().
func checkGetObjectEquality(want interface{}) objectCheckFunc {
	return func(t *testing.T, name string, s SyncGetter) {
		got := s.GetObject()
