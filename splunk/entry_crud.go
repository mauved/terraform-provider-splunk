...
// createFuncForHandler returns a schema.CreateFunc for the ResourceObjectSynchronizer returned by
// the given function.
//
...
func createFuncForHandler[T any](fn func(*T) resource.Sync, legacyFunction schema.CreateFunc) schema.CreateFunc {
	return func(d *schema.ResourceData, meta interface{}) error {
		...
		entry := new(T)
		if err := fn(entry).SyncObject(d); err != nil {
			return err
		}
...
// createFuncForHandlerFunc returns a schema.CreateFunc for the ResourceObjectSynchronizer returned by
// the given function.
//
// During the transition between the legacy and external Splunk clients, it will return legacyFunction
// if the provider configuration or the resource configuration sets use_legacy_client=true.
func readFuncForHandler[T any](fn func(*T) resource.Sync, legacyFunction schema.CreateFunc) schema.ReadFunc {
	return func(d *schema.ResourceData, meta interface{}) error {
		provider := meta.(*SplunkProvider)
		...
		if err := fn(entry).SyncResource(d); err != nil {
			return err
		}
...
// updateFuncForHandler returns a schema.UpdateFunc for the ResourceObjectSynchronizer returned by
// the given function.
//
// During the transition between the legacy and external Splunk clients, it will return legacyFunction
// if the provider configuration or the resource configuration sets use_legacy_client=true.
func updateFuncForHandler[T any](fn func(*T) resource.Sync, legacyFunction schema.CreateFunc) schema.UpdateFunc {
	return func(d *schema.ResourceData, meta interface{}) error {
		provider := meta.(*SplunkProvider)
...
// deleteFuncForHandler returns a schema.DeleteFunc for the ResourceObjectSynchronizer returned by
// the given function.
//
// During the transition between the legacy and external Splunk clients, it will return legacyFunction
// if the provider configuration or the resource configuration sets use_legacy_client=true.
func deleteFuncForHandler[T any](fn func(*T) resource.Sync, legacyFunction schema.CreateFunc) schema.DeleteFunc {
	return func(d *schema.ResourceData, meta interface{}) error {
		provider := meta.(*SplunkProvider)
